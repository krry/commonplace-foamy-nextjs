...
10. Set up the server to auto-run your ghost app
    find `YOUR_NODE_PATH` with this:
    which node
...
sudo service nginx restart 14. Then set up the server to forever run your app even after deploys and power cycles. you can `apt-get pm2` or `apt-get supervisor` or `apt-get forever`. I choose `supervisor` which took some doing as the Ghost docs weren't spot on, but after not doing this, I did this, which worked:
...
supervisorctl -c /etc/supervisor/supervisord.conf start ghost 15. Pull the ripcord. these make sure the ghost floats on its own:
sudo update-rc.d ghost defaults
sudo update-rc.d ghost enable
sudo adduser <your_admin_server_username> ghost 16. Safen your haven. Get an SSL certificate from some sleazy cryptographer on the virtual street corner.
copy your greasy little cert to the system clipboard from a super secure plaintext email and paste into a hidden spot on your server:
vi /etc/ssl/certs/server.crt
then put that stankin' cert in its place:
mkdir /etc/nginx/ssl
cp /etc/ssl/certs/server.crt /etc/nginx/ssl/server.crt 17. Build the ghost app on the server
