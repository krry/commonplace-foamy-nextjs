...
function combForTerms(content, terms) {
	if (terms.length > 0) {
		// keep those terms
	}
	return content
...
export async function patchMissingFrontMatter(dirPath) {
	...
	files.forEach(file => {
		...
		// if there is no terms field, make one
		// if there is a terms field, preserve its data
		// and fill it with any terms in the content
		data.terms = combForTerms(content, data.terms || null)
